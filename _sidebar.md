# Report index
* [Group information](home)
* [DL1. RESTful API description](RESTTful-API-description)
* [DL2. Database design and implementation](Database-design-and-implementation)
* [DL3. RESTful API implementation](restful-api-implementation)
* [DL4. RESTful API documentation and hypermedia](RESTful-API-documentation-and-hypermedia)
* [DL5. API Comsumption (Client + Auxiliary service](API-consumption)
* [FINAL. Reflection and evaluation](analysis-and-reflection)
* [Project Management and Exercises](project_management_and_exercises)

